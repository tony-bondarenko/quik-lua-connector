...
function qsfunctions.getClassSecurityInfo(msg)
	local class_code = msg.data
	local security_collection = split(getClassSecurities(class_code), ",")
	local result = {}
	for i=1,#security_collection do
		security = getSecurityInfo(class_code, security_collection[i])
		result[security.stock_code] = {tiker=security.stock_code, currency=security.trade_currency, code=security.sec_code}
	end
	msg.data = result
	return msg
end
...
function qsfunctions.cancelAllOrders(msg)
	local order_num = 0
	for i = 0, getNumberOf("orders") - 1 do
		local order = getItem("orders", i)
		if bit.test(order.flags, 0) then
			local res = sendTransaction({
				["ACTION"] = "KILL_ORDER",
